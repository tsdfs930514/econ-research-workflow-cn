...
**状态**: 已实现 (2026-02-27 至 2026-02-28)

### 写作工具 (v0.13-cn)

新增 4 个写作技能，完善论文写作流水线：

| 技能 | 范围 |
|-------|-------|
| `/translate` | 中英文学术互译，含术语表和期刊风格适配 |
| `/polish` | 5 种子模式：英文润色、中文润色、精炼（结构性重写）、缩写、扩写 |
| `/de-ai` | AI 写作痕迹检测与消除，含词汇/结构/语义分析 |
| `/logic-check` | 终稿红线审查——仅捕捉致命逻辑错误，不涉及风格偏好 |

### 技能审计 (v0.14-cn)

按 skill-creator 最佳实践审计全部 34 个技能，更新 8 个：

**A 部分 — 写作技能（重大变更）：**

| 技能 | 变更内容 |
|-------|---------|
| `/translate` | 移除 2 处角色扮演语句；重排步骤（术语在模式之前）；自我审查改为检查清单；增加论文惯例说明 |
| `/polish` | 移除 5 处角色扮演语句；增加模式选择指南表；明确 `refine` 范围；翻译部分增加用途说明 |
| `/de-ai` | 移除 1 处角色扮演语句；增加误报指导；增加语境敏感性提示；增加中文"通过"输出模板 |
| `/logic-check` | 歧义部分增加具体示例；明确显著性边界情况；明确样本量匹配规则 |

**B 部分 — 现有技能（描述改进）：**

| 技能 | 变更内容 |
|-------|--------|
| `/explore` | 描述更新，增加触发场景（假设检验、替代规范） |
| `/promote` | 描述更新，增加沙盒到流水线的上下文 |
| `/learn` | 生成的技能模板增加缺失的 `description:` 字段 |
| `/robustness` | 描述更新，增加具体检验类型 |

26 个技能审计确认为 A 级——无需变更。

### 变更的文件

- 8 个技能文件更新：`translate.md`、`polish.md`、`de-ai.md`、`logic-check.md`、`explore.md`、`promote.md`、`learn.md`、`robustness.md`
- 4 个文档文件更新：`README.md`、`CLAUDE.md`、`ROADMAP.md`、`MEMORY.md`

---

## 第七阶段后更新 (v0.16-cn – v0.19-cn)

**状态**: 已实现 (2026-03-02 至 2026-03-06)

### v0.16-cn — CSMAR 技能与钩子/表格修复 (2026-03-02)

- 新增 `/fetch-csmar` 技能——CSMAR API 集成，含浏览/查询/下载/计数模式、自动分页、数据来源追踪、utf-8-sig 编码
- CLAUDE.md 中 Python 核心包新增 `csmarapi`，含使用示例
- 修复 PreCompact 钩子：将不支持的 `"type": "prompt"` 改为 `"type": "command"` + echo
- `/make-table`：5 条 esttab→LaTeX 兼容规则（fragment 模式、`\label{}`、数学模式 R²、下划线转义、threeparttable）
- `/compile-latex`：强化 4 遍编译要求，含逐遍说明表

### v0.17-cn — Stata 日志去重 (2026-03-02)

- `run-stata.sh`：检查日志时优先使用 `output/logs/`；自动删除 Stata `-e` 模式在根目录创建的重复 `.log` 文件
- `stata-log-check.py`：扫描时优先使用 `output/logs/`（规范位置）而非项目根目录

### v0.18-cn — 原始数据目录重构 (2026-03-04)

- 将 `data/raw/` 从 `vN/` 内部移至项目根目录，跨所有版本共享
- 原始数据是不可变的且与版本无关——在每个 `vN/` 中复制浪费存储且语义不正确
- 更新文件：`init-project.md`（目录结构、master.do `$raw` 路径）、`CLAUDE.md`（目录规范、数据安全规则）、`README.md`（快速上手、项目结构）、`replication-standards.md`、`WORKFLOW_QUICK_REF.md`、物理模板

### v0.19-cn — 文档同步 (2026-03-06)

修正 README.md 中与实际代码库不一致的特性计数和缺失条目：

| 项目 | 修正前 | 修正后 | 根因 |
|------|--------|--------|------|
| 技能数量 | 34 | 35 | `/fetch-csmar` 在 v0.16-cn 添加但未反映在 README 中 |
| 规则数量 | 7（4 路径限定 + 3 常驻） | 8（4 路径限定 + 4 常驻） | `bash-conventions.md` 在 v0.15-cn 添加但特性行未更新 |
| 钩子数量 | 3 | 4 | `raw-data-guard.py` 在 v0.15-cn 添加但特性行未更新 |
| 技能表 | 缺少 `/fetch-csmar` | 已补充 | 技能文件存在但 README 表格未更新 |
| 目录结构 | "34 个斜杠命令技能" | "35 个斜杠命令技能" | 同技能数量 |
| 更新日志 | 缺少 3 个 commit | 补充 v0.16-cn – v0.18-cn | Git 历史未反映在 README 更新日志中 |

### 变更的文件

- `README.md` — 特性计数、技能表、目录结构、更新日志（v0.16-cn – v0.19-cn）
- `ROADMAP.md` — 新增本节
...
| 第七阶段 | 完成 | 第六阶段完成；写作工具已添加并审计 |
| 第七阶段后更新 | 完成 | 第七阶段完成；CSMAR 技能、日志修复、数据重构、文档同步 |

...
| 2026-02-28 | 技能审计（第七阶段）：按 skill-creator 最佳实践审计全部 34 个技能 + 1 个参考文档。26 个确认 A 级。更新 8 个技能：(1) /translate — 移除 2 处角色扮演语句、重排步骤、自我审查改检查清单、增加论文惯例说明；(2) /polish — 移除 5 处角色扮演语句、增加模式选择指南、明确 refine 范围、增加用途说明；(3) /de-ai — 移除 1 处角色扮演语句、增加误报指导和语境敏感性提示、增加中文"通过"模板；(4) /logic-check — 增加具体示例、显著性边界和样本量说明；(5-8) /explore、/promote、/learn、/robustness — 描述改进和模板修复。同步更新 README、CLAUDE.md、ROADMAP、MEMORY。中英文两个仓库共 24 个文件编辑。 |
